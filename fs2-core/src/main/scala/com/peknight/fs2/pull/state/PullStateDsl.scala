...
trait PullStateDsl[I, O, S, E]:
  ...
  final type AUX[F[_], A] = PullState[F, I, O, S, E, A]
  ...
  def pure[F[_], A](a: A): AUX[F, A] = StateT.pure[M[F], ST[F], A](a)

  def unit[F[_]]: AUX[F, Unit] = pure(())

  def get[F[_]]: AUX[F, ST[F]] = StateT.get[M[F], ST[F]]

  def getS[F[_]]: AUX[F, S] = get.map(_._1)

  def setS[F[_]](s: S): AUX[F, Unit] =
    for
      (_, stream) <- get
  ...
  def liftPE[F[_], A](pull: Pull[F, O, Either[E, A]]): AUX[F, A] = StateT.liftF(pull)
  def liftP[F[_], A](pull: Pull[F, O, A]): AUX[F, A] = liftPE(pull.map(_.asRight[E]))
  def liftPL[F[_], A](pull: Pull[F, O, E]): AUX[F, A] = liftPE(pull.map(_.asLeft[A]))

  def liftFE[F[_], A](f: F[Either[E, A]]): AUX[F, A] = liftPE(Pull.eval(f))
  def liftF[F[_], A](f: F[A]): AUX[F, A] = liftP(Pull.eval(f))
  def liftFL[F[_], A](f: F[E]): AUX[F, A] = liftPL(Pull.eval(f))

  def liftE[F[_], A](either: Either[E, A]): AUX[F, A] =
    either match
      case Right(value) => pure(value)
      case Left(error) => liftL(error)
  def liftL[F[_], A](e: E): AUX[F, A] = liftPL(Pull.pure(e))
  ...
  def liftFLT[F[_], A](f: F[Throwable])(error: (S, Throwable) => E): AUX[F, A] =
    liftPLT(Pull.eval(f))(error)
  def liftET[F[_], A](either: Either[Throwable, A])(error: (S, Throwable) => E)
  : AUX[F, A] =
    liftPET(Pull.pure(either))(error)
  def liftT[F[_], A](t: Throwable)(error: (S, Throwable) => E): AUX[F, A] =
    liftPLT(Pull.pure(t))(error)

  def output[F[_]](chunk: Chunk[O]): AUX[F, Unit] = liftP(Pull.output(chunk))

  def output[F[_]](os: O*): AUX[F, Unit] = output(Chunk(os*))

  def output1[F[_]](o: O): AUX[F, Unit] = liftP(Pull.output1(o))

  def pipe[F[_]](pipe: Pipe[F, I, O]): AUX[F, Unit] =
    apply((state, stream) => stream.through(pipe).pull.echo.as(((state, Stream.empty), ()).asRight[E]))

  def typed[F[_], A, B: ClassTag](a: A)(f: (S, A) => E): AUX[F, B] =
    a match
      case b: B => pure(b)
      case a => getS.flatMap(s => liftL(f(s, a)))

  def typedS[F[_], A: ClassTag](f: S => E): AUX[F, A] =
    getS.flatMap {
      case a: A => pure(a)
      case s => liftL(f(s))
    }
  ...
  extension [F[_], A] (state: AUX[F, A])
    def attempt(error: (S, Throwable) => E): AUX[F, A] =
    ...
    def outputE(f: Either[E, A] => Chunk[O]): AUX[F, A] =
    ...
    def output(f: A => Chunk[O]): AUX[F, A] =
      state.flatMap(a => this.output(f(a)).as(a))
    def outputL(f: E => Chunk[O]): AUX[F, A] =
      outputE {
