...
class TarFlatSpec extends AsyncFlatSpec with AsyncIOSpec:
  "Tar" should "pass" in {
    ...
    val subFile1: Path = dir / Path("sub-tar-file-1.txt")
    val subFile1Content: String = "file2"
    val subFile2: Path = dir / Path("sub-tar-file-2.txt")
    val subFile2Content: String = "file3"
    val subDir: Path = dir / Path("sub-tar-dir")
    val subSubFile: Path = subDir / Path("sub-sub-tar-file.txt")
