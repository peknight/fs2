...
package object tar:
  def archive[F[_]: {Sync, Files}]: Pipe[F, Path, TarArchiveEntry[F]] =
    archivers.archive[F, TarArchiveEntry[F]](TarArchiveEntry.from[F])

  def readAll[F[_]: Async](chunkSize: Int = 1024 * 32): Pipe[F, TarArchiveEntry[F], Byte] =
    archivers.readAll[F, TarArchiveOutputStream, ApacheTarArchiveEntry, TarArchiveEntry[F]](chunkSize)(
